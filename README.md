...
The videos of bundles of tasks from each participant and angle were grouped into 102 training videos and 44 testing videos. The file train_test_split.yml contains two lists of paths to the training and testing videos respectively. Some of the frames of the test videos are annotated to segment functional parts. Also for the test videos, annotations of the timeframe during which a task is completed and affordances manifest are available in csv format, allowing to evaluate systems that would perform event segmentation. Annotation classes are described in functional_part_labels.yml. Annotations are given as pairs of files: a frame in png format, and a text file where each line is a polygon annotation. A polygon annotation is a class name (string) followed by a floats representing the x, y coordinates of the polygon vertices. The first and last point are identical.
